...
| Startup rollback + shutdown закрывают все ресурсы независимо и идемпотентно | `TelegramBot.run/shutdown`, `WebAppServer.start` | Частичный shutdown | Не было rollback при ошибке запуска Web App/scheduler и закрытия calendar providers | `test_run_cleans_up_when_*`, `test_start_rolls_back_http_server_when_thread_start_fails`, `test_shutdown_failure_does_not_skip_other_resources` |
...
| Per-chat serialization не накапливает lock на каждый когда-либо увиденный chat_id | `ChatLockManager` | Неограниченный dict | Нужен bounded-memory контракт | `test_chat_lock_manager_uses_bounded_stripes` |
...
| Offset watermark не регрессит и не публикуется в memory до durable commit | `offset_store.py`, `offset_tracker.py` | Проверялась монотонность | Не было fault injection записи и retry completed prefix | `test_offset_store_write_failure_keeps_memory_and_disk_unchanged`, `test_offset_tracker_retries_completed_prefix_after_write_failure` |
| CalDAV имеет конечный request timeout, credential-aware потокобезопасный cache и закрывает HTTP-сессии | `caldav_client.py`, `providers/mailru.py`, `user_calendar_service.py` | Частично: fetch/PARTSTAT | Не было lifecycle/cache concurrency контрактов | `test_discovery_uses_finite_timeout_and_close_is_idempotent`, `test_service_cache_is_threadsafe_and_credential_aware`, `test_close_closes_cached_providers_once` |
...
### Дополнительные проверки границ сервисов — 2026-09-12

Добавлены 34 случая без изменений production-кода:

- `test_telegram_transport.py`: временные HTTP/сетевые ошибки, постоянные отказы,
  ограничение количества повторов, выбор задержки HTTP 429, невалидный JSON,
  ошибочный ответ с HTTP 200, маскирование токена и сохранение клавиатуры при
  последовательном отказе эффекта и HTML. Выполняется настоящий код клиента;
  подменяется HTTP-транспорт, задержки и jitter управляемые.
- `test_caldav_range_failures.py`: выбор календаря без fallback на чужой,
  включительные границы дат, сортировка и URL из настоящего ICS, отказ одного
  календаря из нескольких, обрыв итератора ответа и повреждённый VEVENT.
  Строгий режим обязан отказать вместо выдачи неполного отчёта; нестрогий
  сохраняет доступные события.
- `test_bot_lifecycle.py`: рост и ограничение backoff, сброс после успешного
  запроса, передача актуального offset, остановка посреди пачки updates и
  прерывание ожидания при shutdown. Без настоящих пауз и сетевых вызовов.

Полный `make coverage`: **1219 passed**, покрытие строк **87,03%**
(до: 86,14%), ветвей **74,74%** (до: 73,28%). Дополнительно выполнены
101 строка и 44 ветви при неизменном production-коде. Живые Telegram/CalDAV
аккаунты в этих тестах не используются.
