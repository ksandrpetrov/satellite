...
Сквозной стенд [`test_product_correctness.py`](../tests/test_product_correctness.py)
проводит настоящие ICS через HTTP REPORT/GET/PUT/DELETE, CalDAV-библиотеку,
провайдер и пользовательский сервис до плана, аналитики, scheduler и Web API.
Проверяет смешанные статусы, общие календари, частичный отказ с восстановлением,
неопределённое создание без дубля, условный ответ на приглашение и конфликт ETag.
Discovery предзаполнен, Telegram заменён тестовым клиентом, все хранилища временные.

В `test_calendar_stats.py` независимая поминутная модель сверяет занятость,
свободное время и пересечения на 300 воспроизводимых наборах с разными статусами.
`test_ical_parser.py` проверяет RRULE/EXDATE/RDATE, перенос экземпляра, DST и
