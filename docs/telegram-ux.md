...
- останавливает background thread после завершения операции и сразу возвращает
  результат — никаких искусственных `sleep` после `fn`.

`sendMessage` гасит typing моментально на клиенте. После `editMessageText`
индикатор «печатает» может ещё несколько секунд висеть в шапке чата (Telegram
держит его до ~5 с после последнего `sendChatAction`) — это допустимая мелочь
UI, ради которой мы НЕ задерживаем выдачу результата пользователю.
