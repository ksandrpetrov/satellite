...
from .event_identity import deduplicate_event_occurrences, event_occurrence_key
from .event_kinds import filter_meetings_for_analytics
from .events import Event, event_occurs_on, user_partstat
...
def build_week_summary(
    ...
    *,
    ...
    for event in unique_events:
        if not any(
            normalize_caldav_event(event, week_start + timedelta(days=offset), tz, login=login)
            is not None
            for offset in range(WORK_WEEK_DAYS)
        ):
            continue
