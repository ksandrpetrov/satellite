...
_HHMM_INPUT_PATTERN = "^(?P<h>\\d{1,2}):(?P<m>\\d{2})$"


def normalize_hhmm_input(value: str | None) -> str | None:
    """Нормализует пользовательский ввод времени к "HH:MM" или возвращает None.

    Принимает "9:00" / "09:00" / "8:30" / "18:25" / "23:59" / "00:00"; минута —
    всегда ровно две цифры. Любые «человеческие» формы вроде "утром", "9 утра",
    "900", "09-00", "25:00", "12:99" считаются невалидными.
