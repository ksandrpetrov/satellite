...
def test_enrich_invitations_refreshes_empty_attendees_before_accepted_verify(monkeypatch):
    """Mail.ru REPORT без ATTENDEE (как «Кто есть кто» 26.05) — фаза 1 GET до verify ACCEPTED."""
    tz = ZoneInfo("Europe/Moscow")
    moment = datetime(2026, 5, 21, 12, 0, tzinfo=tz)
    login = "alexandra@vk.team"
    target_url = "https://fake/calendars/cal/may26.ics"
    may26 = {
        "summary": "«Кто есть кто»: команды модерации",
        "url": target_url,
        "dtstart": "2026-05-26T17:30:00+03:00",
        "dtend": "2026-05-26T18:30:00+03:00",
        "attendees": [],
    ...
    fillers = [
        {
            "summary": f"Filler {i}",
            "url": f"https://fake/calendars/cal/f{i}.ics",
        ...
        for i, day in enumerate(range(22, 30), start=1)
    ]
